...
const addItem = asyncHandler(async (req, res) => {
  ...
  // Determine image URL: extract from product link or use placeholder
  let imageUrl = "";
  if (productLink) {
    // Try to extract image from the product URL
    console.log(`[addItem] Extracting image from product URL: ${productLink}`);
    const extractedImage =
      await imageExtractor.extractImageFromUrl(productLink);
    if (extractedImage) {
      ...
      console.log(`[addItem] Image extraction failed, using placeholder`);
    }
  } else {
    // Use placeholder if no product URL provided
    imageUrl = imageGenerator.generatePlaceholderImage(req.body.name);
  }
